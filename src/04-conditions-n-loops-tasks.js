...
function getDigitalRoot(num) {
  ...
  if (sum > 9) {
    const meArr = String(sum).split('');
    sum = meArr.reduce((a, b) => Number(a) + Number(b));
    return sum;
  }
  return sum;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    const str = pathes[i].slice(0, end);
    if (!pathes[0].includes(str)) {
      end -= 1;
      path = pathes[0].slice(0, end);
      if (path === '') {
        return path;
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m2[0].length; j += 1) {
      ...
      rowMatrixes.push(num);
    }
    myMatrixes.push(rowMatrixes);
  }
  return myMatrixes;
}
